...
            await connection.query("INSERT INTO requisition_items (requisition_id, item_sl_no, item_name, item_code, description, quantity, unit, freight_required, delivery_location, drawing_url, specimen_url, status, created_by, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'Pending Approval', ?, NOW())", 
            [reqId, i + 1, item.ItemName, item.ItemCode, item.Description, item.Quantity, item.Unit, item.FreightRequired, item.DeliveryLocation, drawingUrl, specimenUrl, req.user.userId]);
        }
...
            const [result] = await connection.query("INSERT INTO bids (item_id, vendor_id, bid_amount, ex_works_rate, freight_rate, comments, bid_status) VALUES (?, ?, ?, ?, ?, ?, 'Submitted')", [bid.item_id, req.user.userId, bid.bid_amount, bid.ex_works_rate, bid.freight_rate, bid.comments]);
            
            await connection.query("INSERT INTO bidding_history_log (bid_id, item_id, vendor_id, bid_amount, ex_works_rate, freight_rate, bid_status, submitted_at) VALUES (?, ?, ?, ?, ?, ?, 'Submitted', NOW())", [result.insertId, bid.item_id, req.user.userId, bid.bid_amount, bid.ex_works_rate, bid.freight_rate]);
        }
        await connection.commit();
...
// BUG FIX 2: Using single quotes for SQL string literals to prevent 'Unknown column' error
...
    try {
        // ITEM NAME ISSUE FIX: Explicitly select item_name from requisition_items
        const query = `
            SELECT 
...
        // ITEM NAME ISSUE FIX: Join with other tables to get all required data
...
                "INSERT INTO requisition_items (requisition_id, item_sl_no, item_name, item_code, description, quantity, unit, freight_required, delivery_location, status, created_by, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'Pending Approval', ?, NOW())",
                [reqId, i + 1, item.ItemName, item.ItemCode, item.Description, item.Quantity, item.Unit, (String(item.FreightRequired).toLowerCase() === 'yes'), item.DeliveryLocation, req.user.userId]
            );
        }
