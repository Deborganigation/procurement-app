...
// CHANGE: Middleware updated to check for both Admin and Super Admin
...
// --- 3. VENDOR FEATURES ---
app.get('/api/requirements/assigned', authenticateToken, async (req, res, next) => { try { const vendorId = req.user.userId; const query = ` SELECT ri.item_id, ri.item_name, ri.item_code, ri.unit, ri.quantity, ri.freight_required, ri.requisition_id, ri.item_sl_no, ri.delivery_location, ri.drawing_url, ri.specimen_url, ( SELECT JSON_ARRAYAGG( JSON_OBJECT( 'ex_works_rate', bhl.ex_works_rate, 'freight_rate', bhl.freight_rate, 'bid_amount', bhl.bid_amount, 'rank', ( SELECT COUNT(DISTINCT b_rank.vendor_id) + 1 FROM bids b_rank WHERE b_rank.item_id = bhl.item_id AND b_rank.bid_amount < bhl.bid_amount ) ) ) FROM bidding_history_log bhl WHERE bhl.item_id = ri.item_id AND bhl.vendor_id = ? ORDER BY bhl.submitted_at ASC ) AS my_bid_history, (SELECT COUNT(*) FROM bidding_history_log WHERE item_id = ri.item_id AND vendor_id = ?) as bid_attempts, CASE WHEN b.bid_id IS NOT NULL THEN (SELECT COUNT(DISTINCT b2.vendor_id) + 1 FROM bids b2 WHERE b2.item_id = ri.item_id AND b2.bid_amount < b.bid_amount) ELSE NULL END AS my_rank FROM requisition_items ri JOIN requisition_assignments ra ON ri.requisition_id = ra.requisition_id LEFT JOIN bids b ON ri.item_id = b.item_id AND b.vendor_id = ? WHERE ra.vendor_id = ? AND ri.status = 'Active' ORDER BY ri.requisition_id DESC, ri.item_sl_no ASC; `; const [items] = await dbPool.query(query, [vendorId, vendorId, vendorId, vendorId]); res.json({ success: true, data: items }); } catch (error) { next(error); }});
...
// CHANGE: Updated API to hide L1 value until item is awarded
...
// CHANGE: Added security logic for Super Admin role change
...
// --- 8. NEW: SUPER ADMIN DELETION ROUTES ---
