...
app.post('/api/admin/reports-data', authenticateToken, isAdmin, async (req, res, next) => {
    try {
        ...
        // KPIs & Analytics
        const [kpisResultRaw, detailedReportRaw, topVendorsRaw, monthlySavingsRaw] = await Promise.all([
            dbPool.query(`
                SELECT
                    SUM(ac.awarded_amount) AS totalSpend,
                    COUNT(DISTINCT ac.item_id) as awardedItemsCount,
                    COUNT(DISTINCT ac.vendor_id) as participatingVendors,
                    SUM(CASE WHEN ac.awarded_amount = (SELECT MIN(bid_amount) FROM bids WHERE item_id = ac.item_id) THEN 1 ELSE 0 END) as l1AwardsCount,
                    SUM(COALESCE(
                        (SELECT MIN(b2.bid_amount) FROM bids b2 WHERE b2.item_id = ac.item_id AND b2.bid_amount > ac.awarded_amount),
                        0
                    )) - SUM(ac.awarded_amount) AS totalSavings
                FROM awarded_contracts ac
                ${dateFilter}`, params),

            dbPool.query(`
                SELECT ac.awarded_amount, ac.awarded_date, ri.requisition_id, ri.item_sl_no, ri.item_name, u.full_name as vendor_name
                FROM awarded_contracts ac
                JOIN requisition_items ri ON ac.item_id = ri.item_id
                JOIN users u ON ac.vendor_id = u.user_id
                ${dateFilter}
            ...
            dbPool.query(`
                SELECT DATE_FORMAT(ac.awarded_date, '%Y-%m') as month,
                       SUM(COALESCE((SELECT MIN(b.bid_amount) FROM bids b WHERE b.item_id = ac.item_id AND b.bid_amount > ac.awarded_amount), 0)) - SUM(ac.awarded_amount) AS monthly_savings
                FROM awarded_contracts ac
                JOIN requisition_items ri ON ac.item_id = ri.item_id
                ${dateFilter}
                GROUP BY month
        ...
        const totalAwards = kpisResult.awardedItemsCount;
        const l1AwardRate = totalAwards > 0 ? (kpisResult.l1AwardsCount / totalAwards) * 100 : 0;

        const totalBidsCountRaw = await dbPool.query(`SELECT COUNT(DISTINCT vendor_id) as total_vendors FROM bids WHERE item_id IN (SELECT item_id FROM awarded_contracts ${dateFilter})`, params);
        const totalBidsCount = totalBidsCountRaw[0][0].total_vendors || 0;
        const vendorParticipationRate = totalAwards > 0 ? (kpisResult.participatingVendors / totalBidsCount) * 100 : 0;

        // Chart Data formatting
        const topVendors = { labels: topVendorsRaw[0].map(v => v.vendor), data: topVendorsRaw[0].map(v => v.total_spend) };
        const savingsTrend = { labels: monthlySavingsRaw[0].map(s => s.month), data: monthlySavingsRaw[0].map(s => s.monthly_savings) };

        res.json({
            success: true,
            data: {
                detailedReport: detailedReportRaw[0],
                kpis: {
                    totalSpend: kpisResult.totalSpend || 0,
                    totalSavings: kpisResult.totalSavings || 0,
                    l1AwardRate: l1AwardRate || 0,
                    vendorParticipationRate: vendorParticipationRate || 0
                },
                charts: {
                    topVendors,
                    savingsTrend
