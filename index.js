...
// --- 3. VENDOR FEATURES ---
app.get('/api/requirements/assigned', authenticateToken, async (req, res, next) => { try { const vendorId = req.user.userId; const query = `SELECT ri.item_id, ri.item_name, ri.item_code, ri.unit, ri.quantity, ri.freight_required, ri.requisition_id, ri.item_sl_no, ri.delivery_location, b.bid_status AS my_bid_status, b.ex_works_rate AS my_ex_works_rate, b.freight_rate AS my_freight_rate, b.bid_amount AS my_bid_amount, (SELECT COUNT(DISTINCT b2.vendor_id) + 1 FROM bids b2 WHERE b2.item_id = ri.item_id AND b2.bid_amount < b.bid_amount) AS my_rank FROM requisition_items ri JOIN requisition_assignments ra ON ri.requisition_id = ra.requisition_id LEFT JOIN bids b ON ri.item_id = b.item_id AND b.vendor_id = ? WHERE ra.vendor_id = ? AND ri.status = 'Active' ORDER BY ri.requisition_id DESC, ri.item_sl_no ASC;`; const [items] = await dbPool.query(query, [vendorId, vendorId]); res.json({ success: true, data: items }); } catch (error) { next(error); }});
app.post('/api/bids', authenticateToken, async (req, res, next) => { if (req.user.role !== 'Vendor') return res.status(403).json({ success: false, message: 'Forbidden' }); let connection; try { connection = await dbPool.getConnection(); const { itemId, ex_works_rate, freight_rate, comments } = req.body; if (!itemId || ex_works_rate === undefined) return res.status(400).json({ success: false, message: 'Item ID and Ex-works rate are required.' }); await connection.beginTransaction(); const [[countResult]] = await connection.query('SELECT COUNT(*) as count FROM bidding_history_log WHERE item_id = ? AND vendor_id = ?', [itemId, req.user.userId]); if (countResult.count >= 3) { await connection.rollback(); return res.status(403).json({ success: false, message: `Maximum of 3 bids reached for this item.` }); } const [[itemDetails]] = await connection.query('SELECT quantity FROM requisition_items WHERE item_id = ?', [itemId]); if (!itemDetails) { await connection.rollback(); return res.status(404).json({ success: false, message: 'Item not found.' }); } const totalBidAmount = (parseFloat(ex_works_rate) + parseFloat(freight_rate || 0)) * parseFloat(itemDetails.quantity); await connection.query('DELETE FROM bids WHERE item_id = ? AND vendor_id = ?', [itemId, req.user.userId]); const [result] = await connection.query("INSERT INTO bids (item_id, vendor_id, bid_amount, ex_works_rate, freight_rate, comments, bid_status, submitted_at) VALUES (?, ?, ?, ?, ?, ?, 'Submitted', NOW())", [itemId, req.user.userId, totalBidAmount, ex_works_rate, freight_rate || 0, comments]); await connection.query("INSERT INTO bidding_history_log (bid_id, item_id, vendor_id, bid_amount, ex_works_rate, freight_rate, bid_status, submitted_at) VALUES (?, ?, ?, ?, ?, ?, 'Submitted', NOW())", [result.insertId, itemId, req.user.userId, totalBidAmount, ex_works_rate, freight_rate || 0]); await connection.commit(); res.json({ success: true, message: 'Bid submitted successfully!' }); } catch (error) { if (connection) await connection.rollback(); next(error); } finally { if (connection) connection.release(); }});
app.get('/api/vendor/my-bids', authenticateToken, async (req, res, next) => { try { const query = `SELECT bhl.*, ri.item_name, ri.requisition_id, ri.item_sl_no, (SELECT COUNT(DISTINCT b2.vendor_id) + 1 FROM bids b2 WHERE b2.item_id = bhl.item_id AND b2.bid_amount < bhl.bid_amount) AS \`rank\` FROM bidding_history_log bhl JOIN requisition_items ri ON bhl.item_id = ri.item_id WHERE bhl.vendor_id = ? ORDER BY bhl.submitted_at DESC`; const [bids] = await dbPool.query(query, [req.user.userId]); res.json({ success: true, data: bids }); } catch (error) { next(error); }});
...
app.get('/api/admin/dashboard-stats', authenticateToken, isAdmin, async (req, res, next) => {
    try {
        const queries = {
            ...
            latestRequisitions: `SELECT r.requisition_id, r.status, r.created_at, u.full_name as creator_name, (SELECT COUNT(*) FROM requisition_items ri WHERE ri.requisition_id = r.requisition_id) as item_count FROM requisitions r LEFT JOIN users u ON r.created_by = u.user_id ORDER BY r.created_at DESC LIMIT 5`,
            ...
            biddingActivity: `SELECT u.full_name, COUNT(b.bid_id) as bid_count FROM bids b JOIN users u ON b.vendor_id = u.user_id GROUP BY u.full_name ORDER BY bid_count DESC LIMIT 5`
        };
        const [ [[activeItems]], [[pendingUsers]], [[awardedContracts]], [[pendingRequisitions]], latestRequisitions, notifications, reqTrends, biddingActivity ] = await Promise.all([
        ...
        res.json({ success: true, data: { activeItems: activeItems.count, pendingUsers: pendingUsers.count, awardedContracts: awardedContracts.count, pendingRequisitionsCount: pendingRequisitions.count, latestRequisitions, notifications, charts: { reqTrends: { labels: reqTrends.map(r => r.month), data: reqTrends.map(r => r.count) }, biddingActivity: { labels: biddingActivity.map(r => r.full_name), data: biddingActivity.map(r => r.bid_count) } } } });
    } catch (error) { next(error); }
});
...
app.post('/api/admin/reports-data', authenticateToken, isAdmin, async (req, res, next) => { try { const { startDate, endDate } = req.body; let dateFilter = ''; const params = []; if (startDate && endDate) { dateFilter = ' WHERE ac.awarded_date BETWEEN ? AND ?'; params.push(startDate, `${endDate} 23:59:59`); } const [kpisResult, detailedReport, vendorSpend, awardedValue, itemSpend, itemFrequency] = await Promise.all([ dbPool.query(`SELECT COALESCE(SUM(ac.awarded_amount), 0) AS totalSpend, COUNT(ac.item_id) as awardedItemsCount, COALESCE(SUM(CASE WHEN ac.awarded_amount <= (SELECT MIN(b.bid_amount) FROM bids b WHERE b.item_id = ac.item_id) THEN 1 ELSE 0 END), 0) as l1AwardsCount, AVG(DATEDIFF(r.approved_at, r.created_at)) as avgApprovalTime, COALESCE(SUM((SELECT bid_amount FROM bids b WHERE b.item_id = ac.item_id ORDER BY b.bid_amount ASC LIMIT 1 OFFSET 1) - ac.awarded_amount), 0) as costSavings FROM awarded_contracts ac JOIN requisition_items ri ON ac.item_id = ri.item_id LEFT JOIN requisitions r ON ri.requisition_id = r.requisition_id ${dateFilter.replace('awarded_date', 'ac.awarded_date')}`, params), dbPool.query(`SELECT ac.awarded_amount, DATE_FORMAT(ac.awarded_date, '%Y-%m-%d') as awarded_date, ri.requisition_id, ri.item_sl_no, ri.item_name, u.full_name as vendor_name FROM awarded_contracts ac LEFT JOIN requisition_items ri ON ac.item_id = ri.item_id JOIN users u ON ac.vendor_id = u.user_id ${dateFilter} ORDER BY ac.awarded_date DESC`, params), dbPool.query(`SELECT u.full_name as vendor, SUM(ac.awarded_amount) as total_spend FROM awarded_contracts ac JOIN users u ON ac.vendor_id = u.user_id ${dateFilter} GROUP BY u.full_name ORDER BY total_spend DESC LIMIT 5`, params), dbPool.query(`SELECT DATE_FORMAT(ac.awarded_date, '%Y-%m') as month, SUM(ac.awarded_amount) as total_awarded FROM awarded_contracts ac ${dateFilter} GROUP BY month ORDER BY month ASC`, params), dbPool.query(`SELECT ri.item_name, SUM(ac.awarded_amount) as total_spend FROM awarded_contracts ac JOIN requisition_items ri ON ac.item_id = ri.item_id ${dateFilter} GROUP BY ri.item_name ORDER BY total_spend DESC LIMIT 5`, params), dbPool.query(`SELECT item_name, COUNT(*) as frequency FROM awarded_contracts ${dateFilter} GROUP BY item_name ORDER BY frequency DESC LIMIT 5`, params) ]); const kpis = kpisResult[0][0]; const totalAwarded = kpis.awardedItemsCount; const l1AwardRate = totalAwarded > 0 ? (kpis.l1AwardsCount / totalAwarded) * 100 : 0; res.json({ success: true, data: { detailedReport: detailedReport[0], kpis: { totalSpend: kpis.totalSpend, l1AwardRate, awardedItemsCount: totalAwarded, avgApprovalTime: kpis.avgApprovalTime || 0, costSavings: kpis.costSavings || 0 }, charts: { vendorSpend: Object.fromEntries(vendorSpend[0].map(row => [row.vendor, parseFloat(row.total_spend)])), awardedValue: Object.fromEntries(awardedValue[0].map(row => [row.month, parseFloat(row.total_awarded)])), itemSpend: { labels: itemSpend[0].map(i => i.item_name), data: itemSpend[0].map(i => i.total_spend) }, itemFrequency: { labels: itemFrequency[0].map(i => i.item_name), data: itemFrequency[0].map(i => i.frequency) } } } }); } catch (error) { next(error); }});
...
app.get('/api/conversations/list', authenticateToken, async (req, res, next) => {
    try {
        ...
        if (req.user.role === 'Vendor') {
            chattableUsersQuery = "SELECT user_id, full_name, role FROM users WHERE role IN ('Admin', 'User') AND is_active = 1";
        } else {
        ...
        if (otherUserIds.length > 0) {
            // FIX: Replaced 'id' with 'message_id' (assuming this is your primary key for messages table)
            const lastMessagesQuery = `
                SELECT 
                    CASE WHEN sender_id = ? THEN recipient_id ELSE sender_id END as other_user_id, message_body, timestamp
