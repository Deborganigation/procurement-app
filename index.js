// ================== DEPENDENCIES ==================
// Yahan hum zaroori packages (libraries) ko import kar rahe hain.
// ===================================================
...
require('dotenv').config(); // Environment variables load karne ke liye (.env file se)

// ================== INITIALIZATION ==================
// Express app ko shuru aur configure kar rahe hain.
// ====================================================
const app = express();
app.use(cors()); // Cross-Origin Resource Sharing enable karna
app.use(express.json({ limit: '50mb' })); // Badi JSON request body allow karna

// SendGrid (Email ke liye) ko API key se set karna
if (process.env.SENDGRID_API_KEY) {
    sgMail.setApiKey(process.env.SENDGRID_API_KEY);
}

// ===== CLOUDINARY CONFIGURATION =====
// Cloudinary (Image/File upload ke liye) ko configure karna
cloudinary.config({
...
// HTML file ko serve karne ke liye setup
...
// ================== DATABASE POOL ==================
// MySQL database se connection ka pool banana.
// ===================================================
const dbConfig = {
...
// SSL Certificate agar environment variable mein hai to use karna
...
// ================== AUTH MIDDLEWARE ==================
// Yeh functions har request se pehle user ko verify karte hain.
// =====================================================

// Token ko verify karne wala middleware
...
// Check karta hai ki user 'Admin' ya 'Super Admin' hai ya nahi
...
// ================== API ROUTES ==================
// Application ke sabhi API endpoints yahan define kiye gaye hain.
// ================================================

// --- 1. AUTH & USER MANAGEMENT ---
// Login aur Register ke liye API endpoints
app.post('/api/login', async (req, res, next) => {
    try {
        ...
        delete user.password_hash; // Response se password hash hatana
...
app.post('/api/register', async (req, res, next) => {
    try {
        ...
        await dbPool.query(
            'INSERT INTO pending_users (full_name, email, password, role, company_name, contact_number, gstin, submitted_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
            [FullName, Email, hashedPassword, Role, CompanyName, ContactNumber, GSTIN, new Date()]
        );
...
// Requisition create karna aur file upload karne ke liye API endpoints

// Cloudinary ke liye storage engine
...
const excelUpload = multer({ storage: multer.memoryStorage() }); // Excel file ko memory mein store karna

// Locations ke liye dropdown data
app.get('/api/dropdowns/locations', authenticateToken, (req, res) => {
    res.json({ success: true, data: ["Dhulaghar", "Kharagpur", "Dankuni", "Kolkata"] });
});

// Nayi Requisition banana (file uploads ke saath)
app.post('/api/requisitions', authenticateToken, upload.any(), async (req, res, next) => {
    let connection;
    try {
        connection = await dbPool.getConnection();
        const { vendorIds, items } = req.body;
        if (!items) return res.status(400).json({ success: false, message: 'No items provided.' });
        ...
        for (const [i, item] of parsedItems.entries()) {
            ...
            await connection.query("INSERT INTO requisition_items (requisition_id, item_sl_no, item_name, item_code, description, quantity, unit, freight_required, delivery_location, drawing_url, specimen_url, status, created_by, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                [reqId, i + 1, item.ItemName, item.ItemCode, item.Description, item.Quantity, item.Unit, item.FreightRequired, item.DeliveryLocation, drawingUrl, specimenUrl, 'Pending Approval', req.user.userId, new Date()]
            );
        }

        if (parsedVendorIds && parsedVendorIds.length > 0) {
            const assignmentValues = parsedVendorIds.map(vId => [reqId, vId, new Date()]);
            await connection.query('INSERT INTO requisition_assignments (requisition_id, vendor_id, assigned_at) VALUES ?', [assignmentValues]);
        }
...
// User ki apni requisitions ka status dekhne ke liye
app.get('/api/requisitions/my-status', authenticateToken, async (req, res, next) => {
    try {
        ...
        const [items] = await dbPool.query(
            `SELECT ri.*, ac.awarded_amount, ac.ex_works_rate, ac.freight_rate, u.full_name as awarded_vendor 
...
// --- 3. VENDOR FEATURES ---
// Vendors ke liye API endpoints (Bids, assigned items, etc.)

// Vendor ko assign kiye gaye items dekhne ke liye
app.get('/api/requirements/assigned', authenticateToken, async (req, res, next) => {
    try {
            ...
            SELECT 
                ...
                (
                    SELECT JSON_ARRAYAGG(
                        JSON_OBJECT(
                            'ex_works_rate', bhl.ex_works_rate, 
                            'freight_rate', bhl.freight_rate, 
                            'bid_amount', bhl.bid_amount,
                    ...
                    WHERE bhl.item_id = ri.item_id AND bhl.vendor_id = ? 
                    ORDER BY bhl.submitted_at ASC
                ) AS my_bid_history,
                (SELECT COUNT(*) FROM bidding_history_log WHERE item_id = ri.item_id AND vendor_id = ?) as bid_attempts,
                CASE 
                    WHEN b.bid_id IS NOT NULL THEN (SELECT COUNT(DISTINCT b2.vendor_id) + 1 FROM bids b2 WHERE b2.item_id = ri.item_id AND b2.bid_amount < b.bid_amount) 
                    ELSE NULL 
                END AS my_rank
            FROM requisition_items ri 
        ...
        `;
        const [items] = await dbPool.query(query, [vendorId, vendorId, vendorId, vendorId]);
...
// Ek saath multiple bids submit karne ke liye (Bulk Bidding)
    ...
    try {
        ...
        for (const bid of bids) {
            ...
            try {
                ...
                const [[itemDetails]] = await connection.query('SELECT quantity, item_name FROM requisition_items WHERE item_id = ?', [itemId]);
                if (!itemDetails) {
                ...
                // Purani bid delete karke nayi daalna
                await connection.query('DELETE FROM bids WHERE item_id = ? AND vendor_id = ?', [itemId, vendorId]);
                const [result] = await connection.query(
                    "INSERT INTO bids (item_id, vendor_id, bid_amount, ex_works_rate, freight_rate, comments, bid_status, submitted_at) VALUES (?, ?, ?, ?, ?, ?, 'Submitted', ?)",
                    [itemId, vendorId, totalBidAmount, ex_works_rate, freight_rate || 0, comments, new Date()]
                );
                
                // History log mein entry
                await connection.query(
                    "INSERT INTO bidding_history_log (bid_id, item_id, vendor_id, bid_amount, ex_works_rate, freight_rate, bid_status, submitted_at) VALUES (?, ?, ?, ?, ?, ?, 'Submitted', ?)",
                    [result.insertId, itemId, vendorId, totalBidAmount, ex_works_rate, freight_rate || 0, new Date()]
                );
                submittedCount++;

            } catch (itemError) {
                console.error(`Error processing bid for item ID ${itemId}:`, itemError);
                const [[itemInfo]] = await connection.query('SELECT item_name FROM requisition_items WHERE item_id = ?', [itemId]).catch(() => [[{item_name: `ID ${itemId}`}]]);
                skippedBids.push(`${itemInfo.item_name} (processing error)`);
            }
        }
        await connection.commit();
...
// Vendor ki sabhi bids ki history dekhne ke liye
app.get('/api/vendor/my-bids', authenticateToken, async (req, res, next) => {
    try {
        const query = `
            SELECT 
                bhl.*, 
                COALESCE(ri.item_name, 'Item Deleted') as item_name, 
                ri.requisition_id, ri.item_sl_no,
            ...
            LEFT JOIN (
                ...
                FROM bids b_inner
                INNER JOIN (
                    SELECT item_id, MIN(bid_amount) as min_bid
                    FROM bids GROUP BY item_id
                ) b_min ON b_inner.item_id = b_min.item_id AND b_inner.bid_amount = b_min.min_bid
...
// Vendor ko mile hue contracts dekhne ke liye
...
// Vendor dashboard ke liye stats
app.get('/api/vendor/dashboard-stats', authenticateToken, async (req, res, next) => {
    try {
        ...
        const recentBidsQuery = `
            ...
            WHERE bhl.vendor_id = ? 
            ORDER BY bhl.submitted_at DESC LIMIT 10`;
...
// Admin aur Super Admin ke liye specific API endpoints

// Admin dashboard ke liye stats
app.get('/api/admin/dashboard-stats', authenticateToken, isAdminOrSuperAdmin, async (req, res, next) => {
    try {
        ...
        const reqTrendsData = { labels: sortedMonths, data: sortedMonths.map(month => trendCounts[month]) };

        res.json({
            success: true,
            data: {
                activeItems: activeItems.count || 0,
                pendingUsers: pendingUsersForNotif.count || 0,
                awardedContracts: awardedContracts.count || 0,
                pendingRequisitionsCount: pendingReqsForNotif.count || 0,
                charts: {
...
// Pending requisitions ko fetch karna
app.get('/api/requirements/pending', authenticateToken, isAdminOrSuperAdmin, async (req, res, next) => {
    try {
        const query = `
            ...
            (SELECT GROUP_CONCAT(u2.user_id, ':', u2.full_name SEPARATOR '||') 
             FROM requisition_assignments ra 
             JOIN users u2 ON ra.vendor_id = u2.user_id 
             WHERE ra.requisition_id = r.requisition_id) as suggested_vendors
            FROM requisitions r 
            LEFT JOIN users u ON r.created_by = u.user_id 
            WHERE r.status = 'Pending Approval' 
            GROUP BY r.requisition_id, r.created_at, u.full_name 
...
// Active requisitions ko fetch karna
    ...
    try {
        const query = `
            SELECT 
                ri.*, 
                l1_details.l1_rate, 
                l1_details.l1_ex_works_rate,
                l1_details.l1_freight_rate,
...
// Ek particular item ke liye sabhi bids ko fetch karna
app.get('/api/items/:id/bids', authenticateToken, isAdminOrSuperAdmin, async (req, res, next) => {
    try {
        const [bids] = await dbPool.query(
            `SELECT b.*, u.full_name as vendor_name, u.email as vendor_email 
             FROM bids b 
             JOIN users u ON b.vendor_id = u.user_id 
             WHERE b.item_id = ? ORDER BY b.bid_amount ASC`, [req.params.id]
        );
...
// Multiple items ke liye bids ko fetch karna (for comparison)
app.post('/api/admin/bids-for-items', authenticateToken, isAdminOrSuperAdmin, async (req, res, next) => {
    try {
        ...
        const [bids] = await dbPool.query(
            `SELECT b.*, u.full_name as vendor_name, u.email as vendor_email 
             FROM bids b 
             JOIN users u ON b.vendor_id = u.user_id 
             WHERE b.item_id IN (?) AND b.bid_status = 'Submitted' 
             ORDER BY b.item_id, b.bid_amount ASC`, [itemIds]
        );
...
app.post('/api/contracts/award', authenticateToken, isAdminOrSuperAdmin, async (req, res, next) => {
    ...
    try {
        ...
        for (const bid of bids) {
            ...
            // Purana awarded contract delete karna
            await connection.query('DELETE FROM awarded_contracts WHERE item_id = ?', [bid.item_id]);
            
            const insertQuery = `INSERT INTO awarded_contracts 
            ...
            await connection.query(insertQuery, [
                ...
                bid.bid_amount, bid.ex_works_rate, bid.freight_rate, bid.bid_id, bid.remarks, new Date()
            ]);
        }
...
// Sabhi awarded contracts ko fetch karna
...
// Reports ke liye data fetch karna
app.post('/api/admin/reports-data', authenticateToken, isAdminOrSuperAdmin, async (req, res, next) => {
    try {
        ...
        const kpisQuery = `
            ...
            FROM awarded_contracts ac ${whereClause}
        `;
        let detailedReportQuery = `SELECT ac.awarded_amount, ac.awarded_date, ri.requisition_id, ri.item_sl_no, ri.item_name, u.full_name as vendor_name FROM awarded_contracts ac LEFT JOIN requisition_items ri ON ac.item_id = ri.item_id JOIN users u ON ac.vendor_id = u.user_id ${whereClause.replace('ac.', 'ac.')} ORDER BY ac.awarded_date DESC`;
        let vendorSpendQuery = `SELECT u.full_name as vendor, SUM(ac.awarded_amount) as total_spend FROM awarded_contracts ac JOIN users u ON ac.vendor_id = u.user_id ${whereClause} GROUP BY u.full_name ORDER BY total_spend DESC LIMIT 5`;
        let awardedValueQuery = `SELECT DATE_FORMAT(ac.awarded_date, '%Y-%m') as month, SUM(ac.awarded_amount) as total_awarded FROM awarded_contracts ac ${whereClause} GROUP BY month ORDER BY month ASC`;
        let itemSpendQuery = `SELECT ri.item_name, SUM(ac.awarded_amount) as total_spend FROM awarded_contracts ac JOIN requisition_items ri ON ac.item_id = ri.item_id ${whereClause} GROUP BY ri.item_name ORDER BY total_spend DESC LIMIT 5`;
        let itemFrequencyQuery = `SELECT item_name, COUNT(*) as frequency FROM awarded_contracts ac ${whereClause} GROUP BY item_name ORDER BY frequency DESC LIMIT 5`;

        const [
            kpisResult, detailedReport, vendorSpend, awardedValue, itemSpend, itemFrequency
        ] = await Promise.all([
            dbPool.query(kpisQuery, params),
            dbPool.query(detailedReportQuery, params),
            dbPool.query(vendorSpendQuery, params),
            dbPool.query(awardedValueQuery, params),
            dbPool.query(itemSpendQuery, params),
            dbPool.query(itemFrequencyQuery, params)
        ]);
        ...
        res.json({
            ...
            data: {
                ...
                charts: {
                    vendorSpend: Object.fromEntries(vendorSpend[0].map(row => [row.vendor, parseFloat(row.total_spend)])),
                    awardedValue: Object.fromEntries(awardedValue[0].map(row => [row.month, parseFloat(row.total_awarded)])),
                    itemSpend: { labels: itemSpend[0].map(i => i.item_name), data: itemSpend[0].map(i => i.total_spend) },
                    itemFrequency: { labels: itemFrequency[0].map(i => i.item_name), data: itemFrequency[0].map(i => i.frequency) }
                }
            }
        });
    } catch (error) {
...
// Bidding ko reopen karna
app.post('/api/items/reopen-bidding', authenticateToken, isAdminOrSuperAdmin, async (req, res, next) => {
    let connection;
    try {
        connection = await dbPool.getConnection();
        const { itemIds } = req.body;
...
// Excel file se bulk requisition upload karna
app.post('/api/requisitions/bulk-upload', authenticateToken, isAdminOrSuperAdmin, excelUpload.single('bulkFile'), async (req, res, next) => {
    ...
    try {
        ...
        for (const [i, item] of items.entries()) {
            await connection.query(
                "INSERT INTO requisition_items (requisition_id, item_sl_no, item_name, item_code, description, quantity, unit, freight_required, delivery_location, status, created_by, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                [reqId, i + 1, item.ItemName, item.ItemCode, item.Description, item.Quantity, item.Unit, (String(item.FreightRequired).toLowerCase() === 'yes'), item.DeliveryLocation, 'Pending Approval', req.user.userId, new Date()]
            );
        }
...
// Poori bidding history dekhne ke liye (Admin)
...
// Ek requisition ke liye assigned vendors ko fetch/update karna
app.get('/api/requisitions/:id/assignments', authenticateToken, isAdminOrSuperAdmin, async (req, res, next) => {
    try {
        const { id } = req.params;
...
app.put('/api/requisitions/:id/assignments', authenticateToken, isAdminOrSuperAdmin, async (req, res, next) => {
    let connection;
    try {
        connection = await dbPool.getConnection();
        const { id } = req.params;
...
// Users ko manage karne ke liye API Endpoints

// Pending user registrations ko fetch karna
app.get('/api/users/pending', authenticateToken, isAdminOrSuperAdmin, async (req, res, next) => {
    try {
...
app.post('/api/users/approve', authenticateToken, isAdminOrSuperAdmin, async (req, res, next) => {
    try {
        ...
        await dbPool.query(
            'INSERT INTO users (full_name, email, password_hash, role, company_name, contact_number, gstin) VALUES (?, ?, ?, ?, ?, ?, ?)',
...
// Sabhi users ki list fetch karna
app.get('/api/users', authenticateToken, isAdminOrSuperAdmin, async (req, res, next) => {
    try {
        const allowedSortColumns = ['full_name', 'role'];
        const sortBy = allowedSortColumns.includes(req.query.sortBy) ? req.query.sortBy : 'full_name';
        const order = req.query.order === 'desc' ? 'DESC' : 'ASC';
        
        const query = `
            SELECT user_id, full_name, email, role, company_name, contact_number, gstin, is_active 
            FROM users 
...
// Sirf vendors ki list fetch karna
...
// Sirf admins ki list fetch karna
app.get('/api/users/admins', authenticateToken, async (req, res, next) => {
    try {
        const [admins] = await dbPool.query("SELECT email FROM users WHERE role IN ('Admin', 'Super Admin') AND is_active = 1");
...
app.put('/api/users/:id', authenticateToken, isAdminOrSuperAdmin, async (req, res, next) => {
    try {
        const { id } = req.params;
        const { full_name, email, role, company_name, contact_number, gstin, password } = req.body;

        if (role === 'Super Admin' && req.user.role !== 'Super Admin') {
            return res.status(403).json({ success: false, message: "Forbidden: Only a Super Admin can assign the Super Admin role." });
        }
...
// Naya user add karna (Admin dwara)
app.post('/api/users/add', authenticateToken, isAdminOrSuperAdmin, async (req, res, next) => {
    try {
        ...
        if (role === 'Super Admin' && req.user.role !== 'Super Admin') {
            return res.status(403).json({ success: false, message: "Forbidden: Only a Super Admin can create another Super Admin." });
        }
...
// User ko apna password set/reset karne ke liye
...
// Real-time messaging aur notifications ke liye endpoints

// Naya message bhejna
app.post('/api/messages', authenticateToken, async (req, res, next) => {
    try {
        const { recipientId, messageBody } = req.body;
        const indianTimestamp = new Date();
        await dbPool.query(
            'INSERT INTO messages (sender_id, recipient_id, message_body, timestamp) VALUES (?, ?, ?, ?)',
            [req.user.userId, recipientId, messageBody, indianTimestamp]
        );
...
// Conversations ki list fetch karna
app.get('/api/conversations/list', authenticateToken, async (req, res, next) => {
    try {
        const myId = req.user.userId;
        
        let chattableUsersQuery;
        if (req.user.role === 'Vendor') {
            chattableUsersQuery = "SELECT user_id, full_name, role FROM users WHERE role IN ('Admin', 'User', 'Super Admin') AND is_active = 1";
        } else {
...
// Ek particular conversation ke messages fetch karna
app.get('/api/messages/:otherUserId', authenticateToken, async (req, res, next) => {
    ...
    try {
        ...
        const [messages] = await connection.query(
            `SELECT *, IF(is_read, 'read', 'sent') as status FROM messages 
             WHERE (sender_id = ? AND recipient_id = ?) OR (sender_id = ? AND recipient_id = ?) 
             ORDER BY timestamp ASC`, [myId, otherUserId, otherUserId, myId]
        );
...
app.post('/api/notifications/mark-all-read', authenticateToken, async (req, res, next) => {
    try {
        await dbPool.query("UPDATE messages SET is_read = 1 WHERE recipient_id = ? AND is_read = 0", [req.user.userId]);
...
// Notifications ki list fetch karna
...
// Sidebar ke liye counts fetch karna (unread messages, etc.)
...
// --- 7. MISC & EMAIL ---
// Chote-mote kaam aur email bhejne ke liye endpoints
...
app.post('/api/send-email', authenticateToken, async (req, res, next) => {
    ...
    const msg = {
        to: recipient,
        from: process.env.FROM_EMAIL, // Yeh email SendGrid par verified hona chahiye
        subject,
        html: htmlBody,
    };
...
// Sirf Super Admin ke liye data delete karne ke endpoints

// Requisition aur usse juda sab data delete karna
app.delete('/api/requisitions/:id', authenticateToken, isSuperAdmin, async (req, res, next) => {
    const { id } = req.params;
    let connection;
    try {
        connection = await dbPool.getConnection();
        ...
        const [items] = await connection.query('SELECT item_id FROM requisition_items WHERE requisition_id = ?', [id]);
        if (items.length > 0) {
            const itemIds = items.map(i => i.item_id);
            await connection.query('DELETE FROM bids WHERE item_id IN (?)', [itemIds]);
            await connection.query('DELETE FROM bidding_history_log WHERE item_id IN (?)', [itemIds]);
            await connection.query('DELETE FROM awarded_contracts WHERE item_id IN (?)', [itemIds]);
        }
        
        await connection.query('DELETE FROM requisition_items WHERE requisition_id = ?', [id]);
        await connection.query('DELETE FROM requisition_assignments WHERE requisition_id = ?', [id]);
        await connection.query('DELETE FROM requisitions WHERE requisition_id = ?', [id]);
        
        await connection.commit();
        res.json({ success: true, message: 'Requisition and all related data deleted successfully.' });
    } catch (error) {
        if (connection) await connection.rollback();
        next(error);
    } finally {
...
app.delete('/api/users/:id', authenticateToken, isSuperAdmin, async (req, res, next) => {
    try {
        const { id } = req.params;
        if (parseInt(id, 10) === req.user.userId) {
            return res.status(400).json({ success: false, message: 'You cannot delete your own account.' });
        }
        await dbPool.query('DELETE FROM users WHERE user_id = ?', [id]);
...
// Pending user registration ko reject/delete karna
app.delete('/api/pending-users/:id', authenticateToken, isSuperAdmin, async (req, res, next) => {
    try {
        const { id } = req.params;
        await dbPool.query('DELETE FROM pending_users WHERE temp_id = ?', [id]);
        res.json({ success: true, message: 'Pending user registration rejected and deleted.' });
    } catch (error) {
        next(error);
    }
});
...
// Agar kisi bhi route mein koi error aata hai, toh yeh use handle karega.
// ==========================================================
