...
                SUM(CASE WHEN ac.winning_bid_id = (SELECT MIN(bid_id) FROM bids WHERE item_id = ac.item_id) THEN 1 ELSE 0 END) as l1AwardsCount,
...
            
        // Charts
        const vendorSpendQuery = `SELECT u.full_name as vendor, SUM(ac.awarded_amount) as total_spend FROM awarded_contracts ac JOIN users u ON ac.vendor_id = u.user_id ${dateFilter} GROUP BY u.full_name ORDER BY total_spend DESC LIMIT 5`;
        const awardedValueByDateQuery = `SELECT DATE_FORMAT(ac.awarded_date, '%Y-%m') as month, SUM(ac.awarded_amount) as total_awarded FROM awarded_contracts ac ${dateFilter} GROUP BY month ORDER BY month ASC`;
